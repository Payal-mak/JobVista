...
$messages = getUserMessages($user_id);
        ...
        <div class="messages-container">
            <div class="messages-sidebar">
                ...
                <div class="messages-list">
                    <?php foreach($messages as $message): ?>
                        <a href="messages.php?employer=<?= $message['sender_id'] == $user_id ? $message['receiver_id'] : $message['sender_id'] ?>" class="message-preview <?= !$message['is_read'] && $message['receiver_id'] == $user_id ? 'unread' : '' ?>">
                            <div class="message-sender">
                                <?= $message['sender_id'] == $user_id ? 'You' : $message['sender_name'] ?>
            ...
            <div class="messages-content">
                <?php if (isset($_GET['employer'])): 
                    $employer_id = sanitize($_GET['employer']);
                    $employer = getUserById($employer_id);
                    $job_id = isset($_GET['job']) ? sanitize($_GET['job']) : null;
                    
                    // Get conversation between user and employer
                    ...
                    $stmt->execute([$user_id, $employer_id, $employer_id, $user_id]);
                    $conversation = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    
                    // Mark all messages as read
                    $stmt = $conn->prepare("UPDATE messages SET is_read = TRUE 
                                          WHERE receiver_id = ? AND sender_id = ?");
                    $stmt->execute([$user_id, $employer_id]);
                ?>
                    <div class="conversation-header">
                        <h3>Conversation with <?= $employer['name'] ?></h3>
                        <?php if ($job_id): 
                        ...
                        <?php foreach($conversation as $msg): ?>
                            <div class="message <?= $msg['sender_id'] == $user_id ? 'sent' : 'received' ?>">
                                <div class="message-header">
                                    <span class="sender"><?= $msg['sender_id'] == $user_id ? 'You' : $msg['sender_name'] ?></span>
                    ...
                    <div class="reply-box">
                        <form method="post" action="../includes/functions.php">
                            ...
                            <input type="hidden" name="receiver_id" value="<?= $employer_id ?>">
